...
- `core/summarizer.py`
  상세 화면용 1~2줄 요약 생성
- `core/todo_extractor.py`
  Okt 형태소 분석 기반 TO-DO 판별 엔진. 마지막 용언의 어미를 분석해 의도(REQUEST / COMMAND / OBLIGATION / FUTURE_INTENT / REPORT)를 분류하고 행동 동사 구조 유무로 최종 판별
- `core/pdf_keywords.py`
  PDF 텍스트와 파일명에서 Okt 어간 기반 키워드 추출
- `core/pdf_related.py`
  TF-IDF 코사인 유사도로 저장된 PDF 간 유사도를 계산하고 추천 목록 생성
...
- `tasks/task_extractor.py`
  메일 1건에서 Task 여러 건 추출. TO-DO 판별은 `core/todo_extractor.py`에 위임
- `tasks/todo_manager.py`
  업무 유형 분류(`classify_task_type`) 및 엔티티 추출, MongoDB 저장
- `tasks/todo_manager_adapter.py`
  외부 호출 인터페이스. 저장은 `mongo_task_store`에, TO-DO 판별은 `todo_extractor`에 위임
...
- PDF 파일명
- PDF 본문 텍스트
- Okt 형태소 분석으로 추출한 어간 키워드 (MongoDB `keywords` 필드에 사전 저장)
- TF-IDF 코사인 유사도 점수 (외부 라이브러리 없이 순수 Python 구현)
